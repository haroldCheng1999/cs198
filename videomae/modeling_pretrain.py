...
class PretrainVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = PretrainVisionTransformerDecoder(
            ...
            use_checkpoint=use_checkpoint)
        # audio
        self.audio_endoder = audioEncoder(
            layers=[3, 4, 6, 3],  num_filters=[16, 32, 64, 128])
        self.audio_proj = nn.Linear(128, decoder_embed_dim, bias=False)
        self.encoder_proj = nn.Linear(
            decoder_embed_dim*2, decoder_embed_dim, bias=False)
        # Self attention
        self.crossA2V = attentionLayer(d_model=decoder_embed_dim, nhead=8)
        self.crossV2A = attentionLayer(d_model=decoder_embed_dim, nhead=8)
        self.selfAV = attentionLayer(d_model=2*decoder_embed_dim, nhead=8)
    ...
    def forward(self, x, audio, mask):
        ...
        # [B, N_mask, 3 * 16 * 16] 24*1568*384
        audio_features = self.audio_endoder.forward(audio)  # 24 *4 *128
        audio_features = self.audio_proj.forward(audio_features)
        audio_features = audio_features.repeat(1, 392, 1)
        # attention
        audio_features = self.crossA2V.forward(audio_features, x_full)
        x_full = self.crossV2A.forward(x_full, audio_features)
        all_features = torch.cat(
            [x_full, audio_features], dim=2)  # 24,1568,192 shape
        all_features = self.selfAV.forward(all_features, all_features)
        all_features = self.encoder_proj.forward(all_features)
        x = self.decoder(all_features, pos_emd_mask.shape[1])
...
class attentionLayer(nn.Module):
    ...
    def forward(self, src: Tensor, tar: Tensor):
