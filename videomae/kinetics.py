...
class VideoClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            if args.num_sample > 1:
                ...
                return frame_list, audio_list, label_list, index_list, {}
            else:
                buffer = self._aug_frame(buffer, args)

            audio = self.load_audio(sample)
            return buffer, audio, self.label_array[index], index, {}
            ...
            audio = self.load_audio(sample)
            return buffer, audio, self.label_array[index], sample.split("/")[-1].split(".")[0]
            ...
            buffer = self.data_transform(buffer)
            audio = self.load_audio(sample)
            return buffer, audio, self.test_label_array[index], sample.split("/")[-1].split(".")[0], \
                chunk_nb, split_nb
        else:
    ...
    @staticmethod
    def load_audio(sample: str):
        cap = cv2.VideoCapture(sample)
        fps = cap.get(cv2.CAP_PROP_FPS)
        file_name = sample.split(".")[0]
        _, audio = wavfile.read(file_name+".wav")
        audio = python_speech_features.mfcc(
            audio, 16000, numcep=13, winlen=0.025 * 25 / fps, winstep=0.010 * 25 / fps)
        audio_tensor = torch.FloatTensor(audio).unsqueeze(0)
        length = audio_tensor.shape[1]
        if length <= 2048:
            audio_tensor_padding = torch.cat(
                [audio_tensor, torch.zeros(1, 2048-length, 13)], dim=1)
        else:
            audio_tensor_padding = audio_tensor[:, :2048, :]
        #print(sample, audio_tensor_padding.shape)
        return audio_tensor_padding
...
class VideoMAE(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio = self.load_audio(directory)
        return (process_data, audio, mask)
    ...
    def _video_TSN_decord_batch_loader(self, directory, video_reader, duration, indices, skip_offsets):
        ...
        try:
            ...
            raise RuntimeError('Error occured in reading frames {} from video {} of duration {}.'.format(
                frame_id_list, directory, duration))
        return sampled_list


class VideoClsDatasetWithAudio(VideoClsDataset):
    def __init__(self, anno_path, data_path, mode='train', clip_len=8, frame_sample_rate=2, crop_size=224, short_side_size=256, new_height=256, new_width=340, keep_aspect_ratio=True, num_segment=1, num_crop=1, test_num_segment=10, test_num_crop=3, args=None):
        super().__init__(anno_path, data_path, mode, clip_len, frame_sample_rate, crop_size, short_side_size,
                         new_height, new_width, keep_aspect_ratio, num_segment, num_crop, test_num_segment, test_num_crop, args)

    def load_audio(self, sample: str):
        cap = cv2.VideoCapture(sample)
        fps = cap.get(cv2.CAP_PROP_FPS)
        file_name = sample.split(".")[0]
        _, audio = wavfile.read(file_name+".wav")
        audio = python_speech_features.mfcc(
            audio, 16000, numcep=13, winlen=0.025 * 25 / fps, winstep=0.010 * 25 / fps)
        return torch.FloatTensor(audio).reshape(1, 1, -1, 1)
