...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.audio_proj = nn.Linear(4*128, embed_dim, bias=False)
    ...
    def forward(self, x, audio):
        x = self.forward_features(x)  # 32*768
        audio_features = self.audio_endoder.forward(audio)  # 32 *4 *128
        audio_features = audio_features.flatten(1, -1)
        audio_features = self.audio_proj.forward(audio_features)  # 32  *768
        all_features = torch.cat(
            [x, audio_features], dim=1)  # 24,1568,192 shape
        all_features = self.encoder_proj.forward(all_features)
        x = self.head(self.fc_dropout(all_features))
        return x
...
class attentionLayer(nn.Module):
    ...
    def forward(self, src: Tensor, tar: Tensor):
        ...
        src = tar + self.dropout1(src2)
        src = self.norm1(src)

        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = tar + self.dropout2(src2)
        src = self.norm2(src)
